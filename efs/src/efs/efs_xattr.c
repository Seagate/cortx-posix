...
/** @TODO: Move this function to a common header file whenever other efs apis use it. */
static inline int efs_ino_to_nodeid(const efs_ino_t *ino, node_id_t *nid)
{
	int rc = 0;
	dassert(ino != NULL);
	dassert(nid != NULL);
		 ...
		 size_t size, int flags)
{
	...
	RC_WRAP_LABEL(rc, out, efs_ino_to_nodeid, ino, &nid);

	if ((flags == XATTR_CREATE) || (flags == XATTR_REPLACE)) {
		RC_WRAP_LABEL(rc, out, md_xattr_exists, &(efs_fs->kvtree->index),
			      &nid, name, &xattr_exists);
	...
	RC_WRAP_LABEL(rc, out, md_xattr_set, &(efs_fs->kvtree->index),
		      &nid, name, value, size);

out:
	log_trace("ctx=%p *ino=%llu nid=%" PRIx64 ":%" PRIx64 " rc=%d name=%s"
		  "val=%p size=%lu", efs_fs, *ino, nid.f_hi, nid.f_lo, rc, name,
		  value, size);
	return rc;
}
		     ...
		     size_t *size)
{
	...
	RC_WRAP_LABEL(rc, out, md_xattr_get, &(efs_fs->kvtree->index),
		      &nid, name,  &read_val, &size_val);
...
out:
	log_trace("ctx=%p *ino=%llu nid=%" PRIx64 ":%" PRIx64 " rc=%d name=%s"
		   " val=%p size=%lu", efs_fs, *ino, nid.f_hi, nid.f_lo, rc,
		    ...
		    const efs_ino_t *ino, const char *name)
{
	...
	RC_WRAP_LABEL(rc, out, md_xattr_delete, &(efs_fs->kvtree->index),
		      &nid, name);

out:
	log_trace("ctx=%p *ino=%llu nid=%" PRIx64 ":%" PRIx64 " rc=%d name=%s",
		   efs_fs, *ino, nid.f_hi, nid.f_lo, rc, name);
	return rc;
}
		  ...
		  size_t *size)
{
	...
	RC_WRAP_LABEL(rc, out, md_xattr_list, &(efs_fs->kvtree->index),
		      &nid, buf, count, size);

out:
	log_trace("ctx=%p *ino=%llu nid=%" PRIx64 ":%" PRIx64 " rc=%d",
		   efs_fs, *ino, nid.f_hi, nid.f_lo, rc);
	return rc;
}
