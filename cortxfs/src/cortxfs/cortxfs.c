...
static int cfs_initialized;

int cfs_init(const char *config_path, const struct cfs_endpoint_ops *e_ops)
{
	...
	/** only initialize cortxfs once */
	if (__sync_fetch_and_add(&cfs_initialized, 1)) {
		return 0;
	}
