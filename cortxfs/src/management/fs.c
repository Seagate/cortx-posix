				     ...
				     void *args)
{
	...
	/* Compose cfs_fs_delete api params. */
	str256_from_cstr(fs_name,
...
static int fs_list_add_entry(const struct cfs_fs_list_entry *fs, void *args)
{
	int rc = 0;
...
static int fs_list_process_request(struct controller_api *fs_list, void *args)
{
	...
	if (fs_list_api->req.fs_name) {
		/* Compose cfs_fs_list api params. */
	...
	cfs_fs_scan_list(fs_list_add_entry, fs_list_api);
