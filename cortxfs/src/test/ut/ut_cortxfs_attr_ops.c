/*
 * Filename: ut_cortxfs_attr_ops.c
...
{
	int rc = 0;
	struct ut_cfs_params *ut_cfs_objs = NULL;

	ut_cfs_objs = calloc(sizeof(struct ut_cfs_params), 1);
	ut_assert_not_null(ut_cfs_objs);
	...
	*state = ut_cfs_objs;
	...
	ut_cfs_objs->file_name = "test_attr_file";
	ut_cfs_objs->file_inode = 0LL;
