/*
 * Filename: ut_cortxfs_link_ops.c
...
static void create_symlink(void **state)
{
	...
	struct ut_cfs_params *ut_cfs_obj = ENV_FROM_STATE(state);
	ut_cfs_obj->file_name = link_name;

	rc = cfs_symlink(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
				&ut_cfs_obj->current_inode, link_name,
	...
	rc = cfs_readlink(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred, &link_inode,
				buff, &content_size);
...
static void create_longname255_symlink(void **state)
{
	...
	struct ut_cfs_params *ut_cfs_obj = ENV_FROM_STATE(state);
	ut_cfs_obj->file_name = long_name;

	ut_assert_int_equal(255, strlen(long_name));

	rc = cfs_symlink(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
				&ut_cfs_obj->current_inode, long_name,
				symlink_content, &link_inode);
...
static int hardlink_setup(void **state)
{
	...
	ut_cfs_obj->file_inode = 0LL;
	ut_cfs_obj->file_name = "test_hardlink_file";
...
static int hardlink_teardown(void **state)
{
	...
	ut_link_obj->ut_cfs_obj.file_inode = 0LL;

	if(ut_link_obj->ut_cfs_obj.file_name != NULL) {
	...
	if(ut_link_obj->link_name != NULL) {
		...
		ut_link_obj->ut_cfs_obj.file_name = ut_link_obj->link_name;
...
static void create_hardlink_delete_original(void **state)
{
	...
	rc = cfs_lookup(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
			&ut_cfs_obj->current_inode, link_name, &link_inode);

	ut_assert_int_equal(rc, 0);

	ut_assert_int_equal(ut_cfs_obj->file_inode, link_inode);

	rc = cfs_unlink(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
			&ut_cfs_obj->current_inode, NULL, ut_cfs_obj->file_name);

	ut_assert_int_equal(rc, 0);

	ut_cfs_obj->file_name = NULL;
...
static void create_hardlink_delete_link(void **state)
{
	...
	rc = cfs_lookup(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
			&ut_cfs_obj->current_inode, link_name, &file_inode);
	...
	rc = cfs_unlink(ut_cfs_obj->cfs_fs, &ut_cfs_obj->cred,
			&ut_cfs_obj->current_inode, NULL, link_name);
